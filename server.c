...
static void handle_get (int connection_fd, const char* page)
{
 ...
 if (*page == '/' && strchr (page + 1, '/') == NULL) {
    char module_file_name[128];
    
        /*separando query string de nome do modulo..(page?para1=val1&param2=val2...)*/ 	
    token = strtok(page, "?"); //token = page
    if(token!=NULL){ /*se houver querystring*/
snprintf (module_file_name, sizeof (module_file_name),
	      "%s.so", token+1);
    token = strtok(NULL, "?");/*token = querystring*/
	}else /*se nao houver qry list apenas copia module_file_name=page*/
    snprintf (module_file_name, sizeof (module_file_name),
	      "%s.so", page+1);
  ...
  if (module == NULL) {
    ...
    (*module->generate_function) (connection_fd, token);/*executa modulo, escreve no socket de acordo com
	token(sehouver)*/
