...
while (pedaco != NULL)
{
    ...
    if (strcmp(param, "tipo") == 0) {
        protocolo = strdup(value);
    }
   pedaco = strtok (NULL, "&"); //aponta proximo pedaco(param=value)
 }
/*preparando argumento*/
char comando[20] = "-a";
if(protocolo==NULL){
protocolo = "tu"; 
strcat(comando, protocolo); //netstat -atu (comando = -atu)
}else{
 strncat(comando, protocolo, 1); //netstat -at OU -au
}
  ...
  if (child_pid == 0) {
    ...
    /* Set up an argumnet list for the invocation of cal.  */
     char* argv[] = { "/bin/netstat", "--numeric-ports", comando, NULL};
